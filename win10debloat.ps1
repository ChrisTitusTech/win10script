...
#This funtion allows multiple things to be installed at the same time no more waiting! :D
function ButtonGet-Process {
    param (
        $name,    
        $package,
        $button
    ...
    $ResultText.text = "`r`n" + "`r`n" + "Installing $name... Please Wait" 
    $scriptblock = ( { param($package) winget install -e $package } )
    Start-Job -Name $name -ScriptBlock $scriptblock -ArgumentList $package
    if ($?) { Write-Host "Installed $name" } # This probably doesn't do anything anymore
    $ResultText.text = "`r`n" + "Finished Installing $name" + "`r`n" + "`r`n" + "Ready for Next Task"
}

$brave.Add_Click({
        $name = "Brave Browser"
        $package = "BraveSoftware.BraveBrowser"
        ButtonGet-Process -name $name -package $package -button $brave
    })

$firefox.Add_Click({
        $name = "Firefox"
        $package = "Mozilla.Firefox"
        ButtonGet-Process -name $name -package $package -button $firefox
    })

$gchrome.Add_Click({
        $name = "Google Chrome"
        $package = "Google.Chrome"
        ButtonGet-Process -name $name -package $package -button $gchrome
    })

$autohotkey.Add_Click({
        $name = "AutoHotkey"
        $package = "Lexikos.AutoHotkey"
        ButtonGet-Process -name $name -package $package -button $autohotkey
    })

$imageglass.Add_Click({
        $name = "Image Glass (Image Viewer)"
        $package = "DuongDieuPhap.ImageGlass"
        ButtonGet-Process -name $name -package $package -button $imageglass
    })

$discord.Add_Click({
        $name = "Discord"
        $package = "Discord.Discord"
        ButtonGet-Process -name $name -package $package -button $discord 
    })

$adobereader.Add_Click({
        $name = "Adobe Reader DC"
        $package = "Adobe.AdobeAcrobatReaderDC"
        ButtonGet-Process -name $name -package $package -button $adobereader
    })

$notepad.Add_Click({
        $name = "Notepad++"
        $package = "Notepad++.Notepad++"
        ButtonGet-Process -name $name -package $package -button $notepad
    })

$vlc.Add_Click({
        $name = "VLC Media Player"
        $package = "VideoLAN.VLC"
        ButtonGet-Process -name $name -package $package -button $vlc
    })

$mpc.Add_Click({
        $name = "Media Player Classic"
        $package = "clsid2.mpc-hc"
        ButtonGet-Process -name $name -package $package -button $mpc
    })

$7zip.Add_Click({
        $name = "7-Zip Compression Tool"
        $package = "7zip.7zip"
        ButtonGet-Process -name $name -package $package -button $7Zip
    })

$vscode.Add_Click({
        $name = "Visual Studio Code"
        $package = "Microsoft.VisualStudioCode --source winget"
        ButtonGet-Process -name $name -package $package -button $vscode
    })

$vscodium.Add_Click({
        $name = "VS Codium"
        $package = "VSCodium.VSCodium"
        ButtonGet-Process -name $name -package $package -button $vscodium
    })
...
$winterminal.Add_Click({
        $name = "New Windows Terminal"
        $package = "Microsoft.WindowsTerminal"
        ButtonGet-Process -name $name -package $package -button $winterminal
    })

$powertoys.Add_Click({
        $name = "Microsoft PowerToys"
        $package = "Microsoft.PowerToys"
        ButtonGet-Process -name $name -package $package -button $powertoys
    })
$everythingsearch.Add_Click({
        $name = "Voidtools Everything Search"
        $package = "voidtools.Everything --source winget"
        ButtonGet-Process -name $name -package $package -button $everythingsearch
    })

$sumatrapdf.Add_Click({
        $name = "Sumatra PDF"
        $package = "SumatraPDF.SumatraPDF"
        ButtonGet-Process -name $name -package $package -button $sumatrapdf
    })

$githubdesktop.Add_Click({
        $name = "GitHub Desktop"
        $package = "GitHub.GitHubDesktop"
        ButtonGet-Process -name $name -package $package -button $githubdesktop
    })

$translucenttb.Add_Click({
        $name = "Translucent Taskbar"
        $package = "TranslucentTB.TranslucentTB"
        ButtonGet-Process -name $name -package $package -button $translucenttb
    })

$etcher.Add_Click({
        $name = "Etcher USB Imager"
        $package = "Balena.Etcher"
        ButtonGet-Process -name $name -package $package -button $etcher
    })

$putty.Add_Click({
        $name = "PuTTY & WinSCP"
        $package = "PuTTY.PuTTY"
        ButtonGet-Process -name $name -package $package -button $putty
    })

$advancedipscanner.Add_Click({
        $name = "Advanced IP Scanner"
        $package = "Famatech.AdvancedIPScanner"
        ButtonGet-Process -name $name -package $package -button $advancedipscanner
    })

$sharex.Add_Click({
        $name = "ShareX Screenshot Tool"
        $package = "ShareX.ShareX"
        ButtonGet-Process -name $name -package $package -button $sharex
    })

$gimp.Add_Click({
        $name = "Gimp Image Editor"
        $package = "GIMP.GIMP"
        ButtonGet-Process -name $name -package $package -button $gimp
    })
...
$essentialtweaks.Add_Click({
        ...
        $services = @(
            "diagnosticshub.standardcollector.service"      #Disables Microsoft (R) Diagnostics Hub Standard Collector Service
            "DiagTrack"                                     #Disables Diagnostics Tracking Service
            "DPS"                                           #Disables DPS?
            "dmwappushservice"                              #Disables WAP Push Message Routing Service (see known issues)
            "lfsvc"                                         #Disables Geolocation Service
            "MapsBroker"                                    #Disables Downloaded Maps Manager
            "NetTcpPortSharing"                             #Disables Net.Tcp Port Sharing Service
            "RemoteAccess"                                  #Disables Routing and Remote Access
            "RemoteRegistry"                                #Disables Remote Registry
            "SharedAccess"                                  #Disables Internet Connection Sharing (ICS)
            "TrkWks"                                        #Disables Distributed Link Tracking Client
            #"WbioSrvc"                                     #Disables Windows Biometric Service (required for Fingerprint reader / facial detection)
            #"WlanSvc"                                      #Disables WLAN AutoConfig
            "WMPNetworkSvc"                                 #Disables Windows Media Player Network Sharing Service
            #"wscsvc"                                       #Disables Windows Security Center Service
            "WSearch"                                       #Disables Windows Search
            "XblAuthManager"                                #Disables Xbox Live Auth Manager
            "XblGameSave"                                   #Disables Xbox Live Game Save Service
            "XboxNetApiSvc"                                 #Disables Xbox Live Networking Service
            "XboxGipSvc"                                    #Disables Xbox Accessory Management Service
            "ndu"                                           #Disables Windows Network Data Usage Monitor
            ...
            "BthAvctpSvc"                                   #Disables AVCTP service (This is Audio Video Control Transport Protocol service.)
            ...
            #Hp services#                                   #
            "HPAppHelperCap"                                #Disables HPAppHelperCap
            "HPDiagsCap"                                    #Disables HPDiagsCap
            "HPNetworkCap"                                  #Disables HPNetworkCap
            "HPSysInfoCap"                                  #Disables HPSysInfoCap
            "HpTouchpointAnalyticsService"                  #Disables HpTouchpointAnalyticsService
            #hyper-v services                               #
            "HvHost"                                        #Disables HvHost
            "vmickvpexchange"                               #Disables vmickvpexchange
            "vmicguestinterface"                            #Disables vmicguestinterface
            "vmicshutdown"                                  #Disables vmicshutdown
            "vmicheartbeat"                                 #Disables vmicheartbeat
            "vmicvmsession"                                 #Disables vmicvmsession
            "vmicrdv"                                       #Disables vmicrdv
            "vmictimesync"                                  #Disables vmictimesync
            # Services which cannot be disabled 
            #"WdNisSvc"                                     #Don't Disable
        )
